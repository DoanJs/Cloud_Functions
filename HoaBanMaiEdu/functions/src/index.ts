...
//             sendTelegram(
//               u.telegramChatId,
//               `💬 <b>${convertPosition(actor.position)}
// ${actor.fullName}</b>` +
...
//   return "\n\n⚠️ Hôm nay là hạn cuối.
// Cô vui lòng hoàn thành ngay trong ngày.";
// }
// /* =====================================================
...
// ${data.teacherBullets.map((item, index) => `${index + 1}.
// ${item}`).join("\n")}
...
export const weeklyBackupFirestoreToTelegram = onSchedule(
  ...
  async () => {
    ...
    const filename = `firestore_backup_HBMEdu_${dateText}.js`;
...
/* =====================================================
   CLOUD FUNCTION UPLOAD IMAGE CLOUDINARY
===================================================== */
...
export const uploadTeacherAvatar = onCall(
  {
    region: "asia-southeast1",
    secrets: [
  ...
  async (request) => {
    ...
    const {teacherId, imageBase64} = request.data;

    if (!teacherId || !imageBase64) {
      throw new HttpsError(
        "invalid-argument",
        "Thiếu teacherId hoặc imageBase64"
      );
    }
    ...
    const [currentUserSnap, teacherSnap] = await Promise.all([
      currentUserRef.get(),
      teacherRef.get(),
    ]);
    ...
    if (!isAdmin && teacherId !== uid) {
      throw new HttpsError(
        "permission-denied",
        "Bạn chỉ được đổi avatar của chính mình"
      );
    }
    ...
    try {
      const uploadResult = await cloudinary.uploader.upload(
        imageBase64,
        {
          folder: "HoaBanMaiEdu/avatars/teacher",
          public_id: teacherId,
          overwrite: true,
          resource_type: "image",
          invalidate: true,
      ...
      await db.collection("Meta").doc("users").set(
        {
  ...
  async (request) => {
    ...
    try {
      const uploadResult = await cloudinary.uploader.upload(
        imageBase64,
        {
          folder: "HoaBanMaiEdu/avatars/children",
          public_id: childId,
          overwrite: true,
          resource_type: "image",
          invalidate: true,
